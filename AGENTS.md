...
# AGENTS.md — Apeiron Storefront

Guide for AI coding agents (Claude, Cursor, Copilot, Kimi, etc.) working in
this repository. Read this before making changes.

## Project

Headless Shopify storefront for the Apeiron clothing brand
(wearapeiron.com). Next.js 16 (App Router, Turbopack, `output: 'standalone'`),
React 19.2, Tailwind CSS 4, TypeScript (strict), Vitest. Source of truth for
products, cart, and checkout is the Shopify Storefront API.
...
npm run lint           # ESLint CLI (next lint was removed in Next 16)
npm test               # vitest run — 34 files / 572 tests, must stay green
npx tsc --noEmit       # typecheck
npm run build          # production build (Turbopack)
```

The CI gate also runs `npm audit --omit=dev --audit-level=high` — it must
report 0 vulnerabilities.

## Environment

- Node.js >= 20.9 (enforced via `engines`; Next 16 dropped Node 18).
- Env vars are server-side only — never `NEXT_PUBLIC_` a secret. The variable
  names in the README table and `.env.example` are the ones the code actually
  reads; keep all three in sync when adding a variable.
- Builds tolerate missing Shopify env vars (catalog routes render their empty
  state), so CI/sandbox builds work without secrets. Do not "fix" the
  resulting build-time warnings.

## Architecture map

- `app/` — routes. Catalog (`shop`, `collection`, `product/[slug]`) is
  static/ISR (`revalidate = 300`); cart/checkout/review and detail routes are
  dynamic. `app/api/` — route handlers (`gate`, `legal`, `review`).
- `proxy.ts` — Next 16's request-interception hook (formerly
  `middleware.ts`): per-request nonce CSP + the pre-launch access gate.
- `lib/` — server modules. `lib/shopify/` is the only Shopify transport
  (native https → curl child-process fallback); `lib/smtp.ts` is a vendored
  TLS-only SMTP client; `lib/rich-text.ts` / `lib/legal-blocks.ts` are
  whitelist HTML parsers.
- `components/`, `hooks/`, `store/` (zustand), `tests/` (vitest).

## Trust invariants — DO NOT BREAK

1. The browser never sends a price. Cart mutations send only
   `merchandiseId` + `quantity`; anything money-adjacent is derived
   server-side from the Shopify cart.
2. Server tokens never cross to the client. Shopify/Judge.me/Resend/SMTP
   secrets are read only in `lib/` server modules (guarded by `server-only`).
3. Failures degrade, they don't throw. Reviews, protection, email, and help
   content resolve to hidden/empty states on third-party failure.

Security posture to preserve: server-side zod re-validation of all input;
SMTP header values CRLF-stripped with RFC 2047 subjects; no
`dangerouslySetInnerHTML` anywhere (whitelist parsers only); link hrefs
protocol-sanitized (`https:`/`http:`/`mailto:`/`tel:`); the gate `?from=`
redirect restricted to internal paths; non-leaking client-facing errors.

## Next.js 16 specifics

- **Request interception is `proxy.ts`** (Node runtime), not `middleware.ts`.
- **All request APIs are async** — always `await cookies()`, `headers()`,
  `params`, `searchParams`. Sync access is a hard error.
- **No `webpack` key in `next.config.ts`** — Turbopack is the default
  bundler and a webpack config fails the build. No `eslint` key either.
- **`revalidateTag(tag)` requires a cacheLife profile** second argument.
- **CSP is split by render mode** (see `proxy.ts` header comment): dynamic
  routes get a strict per-request nonce policy; static/ISR routes get
  `'unsafe-inline'` because a per-request nonce can never match cached HTML.
  When adding a route, check its render mode in the `next build` output
  (ƒ vs ○) and update `isDynamicRoute()` — the default is lenient.
- The block between `NEXT-AGENTS-MD` markers below is a generated Next.js
  16.3 docs index pointing at `./node_modules/next/dist/docs`. Treat your
  memory of Next.js as stale: search those docs before any Next-related task.
  Regenerate with `npx @next/codemod agents-md --output AGENTS.md` — keep
  this project guide above the marker intact.
...
- **ESLint is pinned to v9** (`^9.39.1`). Do not upgrade to ESLint 10:
  ...
  eslint-config-next ships a compatible plugin.
- The one remaining lint warning (`<img>` in `components/RichText.tsx`) is
  pre-existing and accepted — merchant-controlled rich-text images are
  rendered unoptimized deliberately.
- The Shopify clients fall back to spawning `curl` per request on Node
  network errors (hardened: async, secrets over stdin, `--fail`, 30 s
  timeout). Fine as a transient fallback; alert if it fires steadily.
- `hooks/use-mobile.ts` uses `useSyncExternalStore` (not useState+effect) —
  the `react-hooks/set-state-in-effect` rule in eslint-config-next 16 rejects
  the old shadcn pattern.
